...
class Actor(nnx.Module):
    ...
    def __call__(self, y):
        ...
        bijector = distrax.Block(distrax.Chain([
            distrax.ScalarAffine((self.u_min + self.u_max) / 2.0,
...
def sac(env, rng, n_steps=20_000, init_steps=100, lr_actor=0.0003,
        lr_critic=0.001, n_buf=100_000, n_batch=256, gamma=0.99, tau=0.005,
        n_monitor=100, wablog=False):
    ...
    def update(t, buf, actor, critic, target, alpha, opt_actor, opt_critic,
        ...
        nnx.update(target, jax.tree.map(lambda x, y:
            tau * x + (1 - tau) * y, state_critic, state_target))

        return {'critic_loss': critic_loss, 'actor_loss': actor_loss,
            'alpha_loss': alpha_loss}

    def ep_end(metrics, x, y, ep_t, ep_r, i, rng):
    ...
    @nnx.jit
    def step(t, x):
        metrics, x, y, ep_t, ep_r, actor, critic, target, alpha, \
            *opt, buf, rngs = x
        ...
        y_, x, r, term, trunk, _ = env.step(rngs(), x, u)
        ep_t += 1
        ep_r += r

        # Save trasition to replay buffer
        buf = jax.tree.map(lambda x, y: x.at[t % n_buf].set(y), buf,
            Transition(y, u, y_, r, term | trunk))

        # Log episode stats, reset episode if goal reached or limit exceeded
        i = t // (n_steps // n_monitor)
        metrics, x, y, ep_t, ep_r = jax.lax.cond(term | trunk, ep_end,
            lambda m, x, y, ep_t, ep_r, *_: (m, x, y, ep_t, ep_r),
            metrics, x, y_, ep_t, ep_r, i, rngs())
        ...
        metrics = jax.lax.cond((t + 1) % (n_steps // n_monitor) == 0,
            monitor, lambda m, *_: m, metrics, t, x)

        return metrics, x, y_, ep_t, ep_r, actor, critic, target, \
            alpha, *opt, buf, rngs
    ...
    metrics, *_ = nnx.fori_loop(0, n_steps, step, (metrics, x, y, 0, 0., actor,
...
if __name__ == "__main__":
    import gymnax
    env = gymnax.environments.Pendulum()
    env.dy = env.obs_shape[0]             # type: ignore
    env.du = env.action_space().shape[0]  # type: ignore
    env.u_min = env.action_space().low    # type: ignore
    env.u_max = env.action_space().high   # type: ignore
